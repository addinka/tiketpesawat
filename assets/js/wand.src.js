	...
	,construct : function(in_wand)
	{
		...
		for(var i=0, len=video_tags.length; i < len; i++)
		{
			...
			else
			{
				...
				{
					video_start_html = '<video controls="controls" ' + video_options.autoplay_html + ' ' + video_options.autoload_html + ' ' + video_options.poster_html + 'id="' + video_options.update_id + '_video">';
...
$m.h.img_rollovers =
{
	...
	{
		$m.animate.opaque(in_id, "down", $m.h.img_rollovers.increments, function()
		{
			$m.id(in_id).src = $m.attr.get(in_id, "data-mjf_img_on");
			$m.animate.opaque(in_id, "up", $m.h.img_rollovers.increments);
		});
	}
